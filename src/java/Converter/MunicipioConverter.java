...
public class MunicipioConverter implements Converter {
    ...
    public void init() {
        try {
            municipiosDB = filtroController.getMunicipiosDB();
        } catch (NullPointerException e) {
            municipiosDB = new Utils.test().getMunicipios();
            filtroController.setMunicipiosDB(municipiosDB);
        }  
    }
    ...
    public Object getAsObject(FacesContext facesContext, UIComponent component, String submittedValue) {
        if (submittedValue.trim().equals("")) {
            ...
            try {
                ...
                List<Municipio> _municipios = municipiosDB;
                for (Municipio m : _municipios) {
                    if ((int) m.getMunicipio()== number) {
                        return m;
                    }
                }
                
            } catch(NumberFormatException exception) {
                throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Conversion Error", "Not a valid Municipio"));
            }
        }
    ...
    public String getAsString(FacesContext facesContext, UIComponent component, Object value) {
        if (value == null || value.equals("")) {
            ...
            return String.valueOf(((Municipio) value).getMunicipio());
        }
    }
}
